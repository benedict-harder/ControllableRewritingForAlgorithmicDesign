    ...
    public class PlanningStateBeams : PlanningState
    {
        ...
        public override List<Dictionary<string, double>> DefinePartParameters()
        {
            ...
            foreach (RuleDefinition rule in this.RulesToBeExecuted)
            {
                ...
                //double length = i % 2 == 0 ? (ProjectParameters.GetParameterValue("FieldLengthX") - width) / nrOfFields : (ProjectParameters.GetParameterValue("FieldLengthY") - width) / nrOfFields;
                double length = 4.7;
